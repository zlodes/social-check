(function ($) {
	...
	$.socialCheck = function (servicesNames) {
		var servicesIsArray = Array.isArray(servicesNames);

		if (!servicesNames || typeof servicesNames != 'string' && !servicesIsArray) {
			console.error('SocialCheck: services must be string of array of strings!');
			return null;
		}
		...
		function checkService(service) {
			...
			img.src = service.domain + service.redirect;

			img.onload = function () {
				dfd.resolve({service, found: true});
			};

			img.onerror = function () {
				dfd.resolve({service, found: false});
			};
			return dfd.promise();
		}
		...
		$.each(allServices, function (index, service) {
			if (servicesNames.indexOf(service.name) !== -1) {
				services.push(checkService(service));
			}
		});

		return $.when.all(services).then(function (results) {
			var res = {};
			$.each(results, function (index, result) {
				res[result.service.name] = result.found;
			});
			return res;
		});
